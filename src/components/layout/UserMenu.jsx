...
export default function UserMenu({items = defaultMenuItems}){
    return(
        <div className="dropdown dropdown-end">
            <div tabIndex={0} role="button" className="btn btn-circle btn-sm bg-primary text-primary-content border-0 hover:bg-primary/70">
            ...
            <ul className="mt-3 z-[1] p-2 shadow menu menu-sm dropdown-content bg-base-200 rounded-box right-0 min-w-max">
                {items.map(({label,onClick,href}, index) => (
                    <li key={label}>
                        <Link to ={href ?? "#"} onClick={onClick} className="hover:bg-base-400 dark:hover:bg-base-300">
                            {label}
                        </Link>
                    </li>
                ))}
